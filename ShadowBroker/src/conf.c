...
extern char *fuzz_db_file;
extern char *fuzz_avoid_clientid;
extern char *fuzz_ping_from_ip;
#endif
	...
	while(fgets_extending(buf, buflen, fptr)){
		...
		if((*buf)[0] != '#' && (*buf)[0] != 10 && (*buf)[0] != 13){
			...
			if(token){
				...
				}else if(!strcmp(token, "fuzz_client_ip")){    //[Fuzz] Get client IP from config file
#ifdef WITH_FUZZER
					if(conf__parse_string(&token, "fuzz_client_ip", &fuzz_client_ip, saveptr))
						return MOSQ_ERR_INVAL;
#endif					
				}else if(!strcmp(token, "fuzz_db_file")){
#ifdef WITH_FUZZER
					if(conf__parse_string(&token, "fuzz_db_file", &fuzz_db_file, saveptr))
						return MOSQ_ERR_INVAL;
#endif					
				}else if(!strcmp(token, "fuzz_avoid_clientid")){
#ifdef WITH_FUZZER
					if(conf__parse_string(&token, "fuzz_avoid_clientid", &fuzz_avoid_clientid, saveptr))
						return MOSQ_ERR_INVAL;
#endif			
				}else if(!strcmp(token, "fuzz_ping_from_ip")){
#ifdef WITH_FUZZER
					if(conf__parse_string(&token, "fuzz_ping_from_ip", &fuzz_ping_from_ip, saveptr))
						return MOSQ_ERR_INVAL;
#endif
