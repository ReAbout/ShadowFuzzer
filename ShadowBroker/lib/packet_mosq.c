...
#endif
{
	...
	if(!mosq->in_packet.command){
		...
		//log__printf(NULL, MOSQ_LOG_NOTICE, "[Fuzz] Packet read byte: %x.", byte);
		if(read_length == 1){
	...
	if(mosq->in_packet.remaining_count <= 0){
		do{
			...
			if(read_length == 1){
				...
				if(read_length == 0){
					log__printf(NULL, MOSQ_LOG_NOTICE, "[Fuzz] Position 11.");
					return MOSQ_ERR_CONN_LOST; /* EOF */
				}
#ifdef WIN32
				errno = WSAGetLastError();
				...
				if(errno == EAGAIN || errno == COMPAT_EWOULDBLOCK){
					...
					switch(errno){
						case COMPAT_ECONNRESET:
							log__printf(NULL, MOSQ_LOG_NOTICE, "[Fuzz] Position 12.");
							return MOSQ_ERR_CONN_LOST;
						default:
							return MOSQ_ERR_ERRNO;
					}
	...
	while(mosq->in_packet.to_process>0){
		...
		if(read_length > 0){
			...
			if(errno == EAGAIN || errno == COMPAT_EWOULDBLOCK){
				...
				switch(errno){
					case COMPAT_ECONNRESET:
						log__printf(NULL, MOSQ_LOG_NOTICE, "[Fuzz] Position 13.");
						return MOSQ_ERR_CONN_LOST;
					default:
						return MOSQ_ERR_ERRNO;
				}
